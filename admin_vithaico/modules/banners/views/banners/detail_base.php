...
<div class="form-group" id="set-width">
    <label class="col-sm-3 col-xs-12 control-label"><?php echo FSText::_('Chiều rộng'); ?></label>
    <div class="col-sm-9 col-xs-12">
        <select class="form-control select2-temp" name="type" id="type">
            <?php
			$width = array(0 => '25', 1 => '50', 2 => '100');
			foreach ($width as $key => $name) {
				$checked = "";
			?>
            <option value="<?php echo $key; ?>" <?php echo $checked; ?>> <?php echo $name . '% diện tích màn hình';  ?>
            </option>
            <?php
				$i++;
			} ?>
...
<?php
TemplateHelper::dt_edit_image(FSText::_('Image'), 'image', URL_ROOT . @$data->image, '', '', FSText::_('(Dùng ảnh JPG ở vị trí này)'));
?>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
        <div class="" style="margin-bottom: 10px;">
            <input class="radio-custom" type="radio" id='check_none' name='area_select' value='none'
                <?php echo (!@$data->listItemid || @$data->listItemid == 'none') ? 'checked="checked"' : ''; ?> />
            <label for="check_none" class="radio-custom-label"><?php echo FSText::_('Không xuất hiện') ?></label>

            <input class="radio-custom" type="radio" id='check_select' name='area_select' value='select'
                <?php echo (@$data->listItemid && @$data->listItemid != 'none' && @$data->listItemid != 'all') ? 'checked="checked"' : ''; ?> />
            <label for="check_select" class="radio-custom-label"><?php echo FSText::_('Lựa chọn') ?></label>

            <input class="radio-custom" type="radio" id='check_all' name='area_select' value='all'
                <?php echo (@$data->listItemid == 'all') ? 'checked="checked"' : ''; ?> />
		...
		?>
        <select data-placeholder="<?php echo FSText::_('Nơi xuất hiện') ?>" name="menus_items[]" size="8"
            multiple="multiple" class='form-control chosen-select-no-results select2-temp listItem'
            <?php echo (!@$data->listItemid || @$data->listItemid == 'none' || @$data->listItemid == 'all') ? 'disabled="disabled"' : ''; ?>>
            <?php
...
?>
<?php
TemplateHelper::dt_edit_text(FSText::_('Mô tả'), 'summary', @$data->summary, '', 650, 450, 1, '(Nếu bạn chọn loại banner là HTML)', '', 'col-sm-3', 'col-sm-9');
