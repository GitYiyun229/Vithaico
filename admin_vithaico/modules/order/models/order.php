...
//			}
        ...
        if (isset($_SESSION[$this->prefix . 'filter3'])) {
            ...
            if ($filter) {
                $filter = (int)$filter - 1;
                $where .= ' AND a.status_kredivo_int =  "' . $filter . '" ';
            }
        }
        if (isset($_SESSION[$this->prefix . 'keysearch'])) {
            if ($_SESSION[$this->prefix . 'keysearch']) {
                ...
                $where .= " AND ( a.id = '" . str_replace('DH', '', $keysearch) . "' 
                            OR username LIKE '%" . $keysearch . "%'  
						    OR recipients_name LIKE '%" . $keysearch . "%' 
                            OR recipients_email LIKE '%" . $keysearch . "%' 
                            OR nhanh_id = '" . $keysearch . "' 
                            OR ssc_id = '" . $keysearch . "' 
                ";
...
//        $row['is_dispute'] = 0;
        $rs = $this->_update($row, 'fs_order', ' id = ' . $order_id);
