...
class ProductsModelsTables extends FSModels
{
    ...
    function __construct()
    {
        $this -> limit = 30;
        $this -> type = 'products';
        $this -> table_name = FSTable_ad::_('fs_'.$this -> type.'_tables');
        $this -> table_filters = FSTable_ad::_('fs_'.$this -> type.'_filters');
        $this -> table_filters_values = FSTable_ad::_('fs_'.$this -> type.'_filters_values');
        $this -> table_products = FSTable_ad::_('fs_products');
        $this -> table_products_cate = FSTable_ad::_('fs_products_categories');
        $this -> fs_tables = FSTable_ad::_('fs_tables');
    ...
    function setQuery()
    {
        ...
        $query = " 	  SELECT DISTINCT(a.table_name) , a.created_table
...
//		}
    ...
    function rename_table($tablename,$table_name_begin){
        ...
        $rows = $db->affected_rows($sql_rename);
        ...
        $sql_change_table = " UPDATE  ".$this -> table_name."	 
								...
								";
        $rows = $db->affected_rows($sql_change_table);
        if(!$rows)
        {
        ...
        $this -> _update($row, $this -> table_filters,'  tablename = "'.$table_name_begin.'"');
        $this -> _update($row, $this -> table_filters_values,'  tablename = "'.$table_name_begin.'"');
        $this -> _update($row, $this -> table_products_cate,'  tablename = "'.$table_name_begin.'"');
        $this -> _update($row, $this -> table_products,'  tablename = "'.$table_name_begin.'"');
        return true;
    }
    ...
    function recal_summary_auto($tablename,$fields_begin){
        ...
        if(!count($fields_ext)){
            if(count($fields_begin)){
                ...
                $this -> _update($row, $this -> table_products);
        ...
        foreach($records as $item){
            ...
            $this -> _update($row, $this -> table_products ,'id='.$item -> record_id);
    ...
    function get_extend_fields_is_main ( $tablename ){
        global $db;
        if ($tablename == $this->table_products || $tablename == '')
            return;
    ...
    function save_new_field($table_name)
    {
        ...
        if($new_field_total)
        {
            ...
            for($i = 0 ; $i < $new_field_total ; $i ++ )
            {
                ...
                if($new_fname){
                    ...
                    if($new_fname && $this -> check_valid_name($new_fname) )
                    {
                        ...
                        $only_filter = FSInput::get('new_only_filter_'.$i);
                        if($only_filter)
                            $only_filter = 1;
                        else 
                            $only_filter = 0;
                        ...
                        if($foreign_id){
                            $foreign = $this -> get_record_by_id($foreign_id,'fs_extends_groups');
                            $new_ftype = $foreign->field_type ? $foreign->field_type : 'varchar';
                            if($new_ftype == 'foreign_one' || $new_ftype == 'foreign_multi'){
                                $foreign_name = $foreign -> name;
                                $foreign_tablename = $foreign -> table_name; 
                            }
                        ...
                        $arr_insert_table[]  = 		"('$table_name','$new_fname','$new_ftype','$new_fshow','$foreign_id','$foreign_name','$foreign_tablename','$is_main','$is_filter','$is_config','$group_id','$ordering','$only_filter')";
            ...
            if(count($arr_insert_table))
            {
                ...
                $sql_insert_table .= "		(table_name,field_name,field_type,field_name_display,foreign_id,foreign_name,foreign_tablename,is_main,is_filter,is_config,group_id,ordering,only_filter) ";
                $sql_insert_table .= "		 VALUES ";
                $sql_insert_table .= implode(",", $arr_insert_table);

                $rs = $db->insert($sql_insert_table);
                if(!$rs)
                {
...
//						return false;
    ...
    function save_exist_field($table_name)
    {
        ...
        for($i= 0 ; $i < $field_extend_exist_total ; $i++)
        {
            ...
            // $fshow_exist = $fsstring -> stringStandart($fshow_exist);

            $fshow_exist_begin = FSInput::get('fshow_exist_'.$i.'_begin');

            // $fname_exist = FSInput::get('fname_exist_'.$i);

            $fname_alias = $fsstring -> stringStandart($fshow_exist);

            $fname_exist = str_replace('-','_',$fname_alias);
            ...
            $group_id_exist = FSInput::get('group_id_exist_'.$i,0);
            ...
            $only_filter = FSInput::get('only_filter_exist_'.$i);
            if($only_filter)
                $only_filter = 1;
            else 
                $only_filter = 0;
                
            $only_filter_begin = FSInput::get('only_filter_'.$i.'_begin');
            ...
            $ftype_exist = 'varchar';
            if($foreign_id){
                $foreign_type = $this->get_record('id = '.$foreign_id,'fs_extends_groups','id,field_type');
                if($foreign_type)
                    $ftype_exist = $foreign_type->field_type;
            }

            if($fname_exist)
            {
                if( ($fname_exist != $fname_exist_begin) || 
                    ($ftype_exist != $ftype_exist_begin) || 
                    ($is_main != $is_main_begin) || 
                    ($is_filter != $is_filter_begin) || 
                    ($is_config != $is_config_begin) || 
                    ($fshow_exist != $fshow_exist_begin) || 
                    ($group_id_exist != $group_id_exist_begin) || 
                    ($foreign_id != $foreign_id_begin) ||
                    ($ordering != $ordering_begin) || 
                    ($only_filter_begin != $only_filter)
                )
                {
                    switch ($ftype_exist)
                    {
                    ...
                    // update filter
                    $sql_filter_update = " UPDATE $this->table_filters SET	field_name = '$fname_exist',field_show = '$fshow_exist',field_ordering = '$ordering' WHERE tablename = '$table_name' AND field_name = '$fname_exist_begin'   ";
                    //						$rs = $db->affected_rows($sql_filter_update);
    ...
    function remove(){
        ...
        if(count($tablenames))
        {
            ...
            // remove filters:
            $this -> _remove(" tablename IN (".$str_tablenames.") ",$this -> table_filters) ;
            $this -> _remove(" tablename IN (".$str_tablenames.") ",$this -> table_filters_values) ;
    ...
    function get_default_fields_in_extends(){
        ...
        return $this -> get_records($where, $this->fs_tables);
    }
