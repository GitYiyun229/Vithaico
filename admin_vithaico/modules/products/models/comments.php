...
//		}
	...
	function ajax_published($published = 1) {
		...
		$rs = $this->_update ( $row, 'fs_products_comments', 'id = ' . $id . ' AND product_id =  ' . $product_id . ' ' );
		echo $rs ? 1 : 0;
		if ($rs) {
	...
	function ajax_del() {
		...
		$rs = $this->_remove ( 'id = ' . $id . ' AND product_id =  ' . $product_id . ' ', 'fs_products_comments' );
		echo $rs ? 1 : 0;
		if ($rs) {
			$this->recal_comments ( $product_id );
		}
	...
	function recal_comments($product_id) {
		$list = $this->get_records ( 'product_id = ' . $product_id, 'fs_products_comments', 'id,published' );
		$total_published = 0;
	...
	function update_unread_for_comments($product_id) {
		$row ['readed'] = 1;
		$this->_update ( $row, 'fs_products_comments', 'product_id = ' . $product_id );
		
		$row2 ['comments_unread'] = 0;
		$this->_update ( $row2, 'fs_products', 'id = ' . $product_id . ' ' );
		return;
	}
