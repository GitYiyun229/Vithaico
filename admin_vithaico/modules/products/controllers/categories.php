...
//        $list_trademark = $model->get_trademark();
        ...
        $priceByCitys = json_decode($data->price_by_city, true);
        include 'modules/' . $this->module . '/views/' . $this->view . '/detail.php';
    }
    ...
    {
        $data = 0;
        $city_id = FSInput::get('id');
        $id_cad = FSInput::get('id_cad');
        $priceByCity = $this->model->get_record('id=' . $id_cad, 'fs_products_categories', 'price_by_city')->price_by_city;
        $priceByCity = json_decode($priceByCity, true);
        foreach ($priceByCity as $key => $price) {
            if ($price['city_id'] != $city_id) {
                $priceByCityNew[$key] = $price;
            }
        }
        $priceByCityNew = json_encode($priceByCityNew,JSON_UNESCAPED_UNICODE );
        $update = $this->model->_update(['price_by_city'=>$priceByCityNew], 'fs_products_categories', 'id = ' . $id_cad, 0);
        if ($update){
            $data = 1;
        }
        echo $data;
