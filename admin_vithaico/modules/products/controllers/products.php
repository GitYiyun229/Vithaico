...
class ProductsControllersProducts extends Controllers
{
    ...
    function edit()
    {
        ...
        $aspects = $model->get_records('product_id =' . $id, 'fs_aspect');
        ...
        $list_products = $model->get_records("published = 1 and id != $id", 'fs_products', 'id, name');
    ...
    function ajax_delete_aspect()
    {
        $status = 0;
        $model = $this->model;
        $id = FSInput::get('fieldid');
        $remove = $model->_remove('id =' . $id, 'fs_aspect');
        if (!empty($remove)) {
            $status = 1;
        }
        echo $status;
    ...
    public function getAvailableAll()
    {
        require PATH_BASE . 'libraries/fscontrollers.php';
        require PATH_BASE . 'modules/api/controllers/ssc.php';

        $link = "index.php?module=$this->module&view=$this->view";

        $products = $this->model->get_records("code != '' AND combo = 1", "fs_products", "id, code, nhanh_id, quantity, combo");

        foreach ($products as $item) {
            $sub = $this->model->get_records("product_id = $item->id", "fs_products_sub", "id, code, product_id, quantity");
            $total = 0;
            if (!empty($sub)) {
                foreach ($sub as $subItem) {
                    $data = $item->combo ? ApiControllersSsc::getComboAvailable($subItem->code) : ApiControllersSsc::getAvailable($subItem->code);
                    $total += $data->available ?: 0;
                    $this->model->_update(['quantity' => $data->available ?: 0], 'fs_products_sub', "id = $subItem->id");
                }
            } else {
                $data = $item->combo ? ApiControllersSsc::getComboAvailable($item->code) : ApiControllersSsc::getAvailable($item->code);
                $total = $data->available ?: 0;
            }

            $this->model->_update(['quantity' => $total], 'fs_products', "id = $item->id");
        }
        
        setRedirect($link, "Cập nhật thành công!", "success");
    }
    ...
    public function getAvailable()
    {
        ...
        $sub = FSInput::get('field_extend_exist_total');
        $combo = FSInput::get('combo', 0, 'int');
        $total = 0;
        $link = "index.php?module=$this->module&view=$this->view&task=edit&id=$id";
        if ($this->page) {
            $link .= '&page='.$this -> page;
        }
        ...
        if ($sub) {
            for ($i = 0; $i < $sub; $i ++) {
                $idSub = FSInput::get("id_exist_$i");
                $codeSub = FSInput::get("code_exist_$i");
                $data = $combo ? ApiControllersSsc::getComboAvailable($codeSub) : ApiControllersSsc::getAvailable($codeSub);

                if ($data->status) {
                    $_SESSION['msg_error'][] = $data->message;
                }

                $total += $data->available ?: 0;
                $this->model->_update(['quantity' => $data->available ?: 0], 'fs_products_sub', "id = $idSub");
            }
        } else {
            if (!$code) {
                setRedirect($link, "Sản phẩm chưa nhập mã!", "error");
            } else {
                $data = $combo ? ApiControllersSsc::getComboAvailable($code) : ApiControllersSsc::getAvailable($code);
        ...
        $message = "Tổng tồn $total";
        
        $this->model->_update(['quantity' => $total], 'fs_products', "id = $id");

        setRedirect($link, "Cập nhật thành công! $message", "success");
    }

    public function getDetail()
    {
        require PATH_BASE . 'libraries/fscontrollers.php';
        require PATH_BASE . 'modules/api/controllers/nhanh.php';

        $id = FSInput::get('id');
        $nhanhId = FSInput::get('nhanh_id');
        $sub = FSInput::get('field_extend_exist_total');
        $type = "success";
        ...
        if ($sub) {
            ...
            for ($i = 0; $i < $sub; $i ++) {
                $idSub = FSInput::get("id_exist_$i");
                $nhanhId = FSInput::get("nhanh_exist_$i");

                if ($nhanhId) {
                    $data = ApiControllersNhanh::productDetail($nhanhId);
                    if ($data->code) {
                        $price[] = $data->data->$nhanhId->price;
                        $this->model->_update(['price' => $data->data->$nhanhId->price], 'fs_products_sub', "id = $idSub");
                    } else {
                        // $message .= " Mã $nhanhId không tồn tại trên Nhanh";
                        $_SESSION['msg_error'][] = "Mã $nhanhId " . $data->messages[0];
                    }
                }
            }

            $price = !empty($price) ? min($price) : 0;
        } else {
            if (!$nhanhId) {
                setRedirect($link, "Cập nhật thành công! Sản phẩm chưa có Nhanh ID", "error");
            }
        ...
        if (isset($price)) {
            $this->model->_update(['price' => $price], 'fs_products', "id = $id");
        }
