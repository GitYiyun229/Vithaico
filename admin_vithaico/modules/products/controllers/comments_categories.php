<?php
class ProductsControllersComments_categories extends Controllers
{
	...
	function ajax_get_comments_by_product()
	{
		...
		$model = $this->model;
		$data = $model->get_comments_by_product($product_id);
		//			var_dump($data);
		$html = $this->genarate_comments($data);
		echo $html;
		if (count($data)) {
			$model->update_unread_for_comments($product_id);
		}
		return;
	}
	...
	function genarate_comments($data)
	{
		...
		foreach ($data as $item) {
			...
			$html .= '<span class="email" >(' . $item->email . ' / ' . $item->phone . ')</span> / Đánh giá :<b>' . $item->rating . ' sao </b></div> ';
			...
			$html .= '		<a class="button" href="javascript: void(0);" onclick="javascript: submit_edit(' . $item->id . ',' . $item->product_id . ')"';
			$html .= '			<span>Gửi</span>	';
			$html .= '		</a>&nbsp;&nbsp;	';
			...
			if ($item->published) {
				$html .= '<a href="javascript:void(0);" onclick="return ajax_unpublished(' . $item->id . ',' . $item->product_id . ')" title="Click vào để ngừng xuất bản">';
				$html .= '<img border="0" src="templates/default/images/published.png" alt="Enabled status"></a>';
			} else {
				$html .= '<a href="javascript:void(0);" onclick="return ajax_published(' . $item->id . ',' . $item->product_id . ')" title="Click vào để xuất bản">';
			...
			$html .= '&nbsp;&nbsp;<a href="javascript:void(0);" onclick="return ajax_del(' . $item->id . ',' . $item->product_id . ',' . $item->published . ')" title="Xóa">';
			...
			$html .= '</div>';
			$html .= '	<div class="reply_button_area">	';
			$html .= '		<a class="button" href="javascript: void(0);" onclick="javascript: submit_reply(' . $item->id . ',' . $item->product_id . ')"';
			$html .= '			<span>Gửi</span>	';
			$html .= '		</a>	';
	...
	{
		$link = 'index.php?module=products&view=comments&keysearch=&text_count=1&text0=' . $product->id . '&filter_count=1&filter0=0';
		$html = '';
