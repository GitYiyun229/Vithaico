<?php 
	class FileModelsFile extends FSModels
	{
		var $limit;
		var $prefix ;
		function __construct()
		{
			...
			$this -> arr_img_paths = array(
                                            array('larger',632,423,'cut_image'),
                                            array('resized',302,203,'cut_image'),
                                            array('small',112,75,'cut_image')
                                        );
			$this -> table_category_name = FSTable_ad::_('fs_news_categories');
            $this -> table_name = FSTable_ad::_('fs_contact_download');
...
//			$row['category_alias'] = $cat -> alias;
      ...
      //       $image_name_icon = $_FILES["icon"]["name"];
            ...
            $file_price = $_FILES["file_download"]["name"];
            if ($file_price) {
                $path_original = $path;
...
//					$this -> remove_file($id,$img_paths,'file_upload');
                ...
                // upload
                $file_price_name = $fsFile->upload_file("file_download", $path_original, 50000000, '_' . time());
                if (!$file_price_name)
                    return false;
                $row['file_download'] = 'images/upload_file/' . $cyear . '/' . $file_price_name;
            }
