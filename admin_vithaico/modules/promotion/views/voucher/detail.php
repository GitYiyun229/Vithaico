...
$this->dt_form_begin(1, 4,'', 'fa-edit', 1, 'col-md-12', 1);
    ...
    $this->dt_form_begin(1, 2, FSText::_('Thông tin cơ bản'), '', 1, 'col-md-12 fl-left');
        ...
        TemplateHelper::dt_edit_text(FSText::_('Mã voucher'), 'code', @$data->code, '', '', '', '', '', '', 'col-md-2', 'col-md-3', @$data ? 1 : 0);
        TemplateHelper::dt_edit_selectbox(FSText::_('Loại voucher'), 'type', @$data->type, 0, [1=>'Voucher freeship', 2=>'Voucher đơn hàng'], $field_value = 'id', $field_label = 'treename', $size = 10, 0, 1,'','','','col-md-2','col-md-3');
            ...
            <div class="col-md-10 col-xs-12">
                <div class="d-flex gap-2 align-items-center">
                    <div class="input-price d-flex">
                        <input class="form-control" name="price" type="text" placeholder="" value="<?php echo @$data->price != 0 ? format_money_0(@$data->price, '', '') : '' ?>" />
                    </div>
                    hoặc
                    <div class="input-percent d-flex">
                        <input class="form-control" name="percent" type="text" placeholder="" value="<?php echo !@$data->price == 0 ? @$data->percent : '' ?>" />
                    </div>
                </div>
            </div>
        </div>
        <?php
        TemplateHelper::dt_edit_text(FSText::_('Số tiền chi tối thiểu'), 'min_amount', format_money_0(@$data->min_amount, '', '0'), 0, '', '', '', '', '', 'col-md-2', 'col-md-3');
        TemplateHelper::dt_edit_text(FSText::_('Số lượng có thể nhận'), 'quantity', @$data->quantity, 1, '', '', '', '', '', 'col-md-2', 'col-md-3');
        TemplateHelper::dt_edit_text(FSText::_('Giới hạn nhận mỗi khách'), 'member_limit', @$data->member_limit, 1, '', '', '', '', '', 'col-md-2', 'col-md-3');
    ...
    ?>
        <a href="" class="add-member" data-toggle="modal" data-target="#memberModal">Thêm Khách hàng</a>
                ...
                <tbody>
                    <?php if (!empty($members)) { ?>
                        <?php foreach ($members as $item) { ?>
                            <tr>
                                <td>
                                    <input name="member_id[]" type="hidden" value="<?php echo $item->id ?>" />
...
<div class="modal fade modal-search" id="memberModal" tabindex="-1" role="dialog" aria-labelledby="memberModalLabel">
...
<style>
    ...
    #productsModal .modal-dialog, #memberModal .modal-dialog{
        margin: 60px auto;
        width: 900px;
    }
    .table-record img{
    ...
    #productsModal .select2, #memberModal .select2{
...
<script type="text/javascript">
    // mảng product-member đã chọn
    let productId = [<?php echo @$data->product_id ?: '' ?>];
    let memberId = [<?php echo @$data->member_id ?: '' ?>];
    ...
    $('.search-add').click(function(){
        ...
        if (modalId == 'memberModal') {
            $(this).closest('.modal-search').find("table tbody input[type=checkbox]:checked").each(function(i, item){
                let data = JSON.parse($(this).attr('data'))
                if (!memberId.includes(parseInt(data.id))) {
                    html += `
                        <tr>
                            <td>
                                <input name="member_id[]" type="hidden" value="${data.id}" />
                    ...
                    `
                    memberId.push(parseInt(data.id)); 
                }
            })
        ...
        $(this).closest('.modal-search').find('table tbody').empty()
        $('.modal-search').modal('hide') 
        $('.search-check-all').prop('checked', false)
    ...
    $(document).on('click', '.delete-product', function(e){
        ...
        if (table.hasClass('table-record')) {
        ...
        if (table.hasClass('table-member')) {
            let index = memberId.indexOf(id);
            if (index !== -1) {
    ...
    function formValidator() {
        ...
        if (!notEmpty('type', 'Vui lòng chọn loại voucher')) {
            return false;
        }

        // if (!$('.table-record tbody tr').length) {
        //     alert ('Vui lòng chọn sản phẩm');
        //     $('#msg_error').text('Vui lòng chọn sản phẩm')
        //     return false
        // }
