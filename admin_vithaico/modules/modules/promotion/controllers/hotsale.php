...
class PromotionControllersHotsale extends Controllers
{
...
//        $promotion_products = $model->get_list_promotion_products($data->promotion_products);
    ...
    function update_product($id)
    {
        ...
        $model = $this->model;
        $promotion = $model->get_record_by_id($id, 'fs_hotsale');
//			if($promotion -> date_start < date('Y-m-d H:i:s') && $promotion -> date_end > date('Y-m-d H:i:s')){
        $link = 'index.php?module=' . $this->module . '&view=hotsale&task=update&id=' . $id;
//			}else{
