...
//	$toolbar->addButton('import',FSText::_('Import'),'','import.png');
	$toolbar->addButton('boss',FSText :: _('Published'),FSText :: _('You must select at least one record'),'published.png');
	$toolbar->addButton('unboss',FSText :: _('Unpublished'),FSText :: _('You must select at least one record'),'unpublished.png');
//	$toolbar->addButton('export',FSText :: _('Export'),'','Excel-icon.png');
//	$toolbar->addButton('',FSText :: _('Export'),FSText :: _(''),'Excel-icon.png','','1');
?>
<div class="form_body">
	<form action="index.php?module=members&view=boss" name="adminForm" method="post" >
...
//			$fitler_config['filter'][] = $filter_slideshow;
		...
		<div class="form-contents">
			<table border="1" class="tbl_form_contents" cellpadding="3" bordercolor="#CCC">
				<thead>
					...
					<th class="title" >
						<?php echo  TemplateHelper::orderTable(FSText::_('Username'), 'a.treename',$sort_field,$sort_direct) ; ?>
					</th>
					<th class="title">
				...
				<tbody>
					...
					<?php if(@$list){?>
						<?php foreach ($list as $row) { ?>
							<?php $link_view = "index.php?module=members&view=boss&task=edit&id=".$row->id.""; ?>
							<tr class="row<?php echo $i%2; ?>">
								<td><?php echo $i+1; ?></td>
								<td>
									<input type="checkbox" onclick="isChecked(this.checked);" value="<?php echo $row->id; ?>"  name="id[]" id="cb<?php echo $i; ?>">
								</td>
								<td align="left"><a href='<?php echo $link_view;?>' ><?php echo $row->username; ?></a></td>
								<td><a href='<?php echo $link_view;?>' ><?php echo $row->full_name; ?></a></td>
								<td><?php echo date("m-d-Y",strtotime($row->created_time)); ?></td>
								<td>
									<?php if($row -> is_boss){?>
										<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','unboss')" title="Disable item">
											<img border="0" src="templates/default/images/published.png" alt="Enabled status"></a>
									<?php }else{?>
										<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','boss')" title="Enabled status">
											<img border="0" src="templates/default/images/unpublished.png" alt="Disable item">
										</a>
									<?php }?>
								</td>
		...
		<input type="hidden" value="boss" name="view">
...
configOpenPopup(1);
	function configOpenPopup(){
		$('.Export').click(function(){
			$.get('index.php?module=members&view=boss&task=quality_export&raw=1',function(response){
...
function configClickExport(){
	$('#submit_quality').click(function(){
		...
		var end=$('#end_at').val();
		$.get('index.php?module=members&view=boss&task=export_excel&raw=1&start='+start+'&end='+end,function(response){
			if(response != "error"){
