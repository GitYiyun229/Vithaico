...
//        $html = $this->products_genarate_related($data);
    ...
    function products_genarate_related($data,$promotion_id)
    {
        ...
        foreach ($data as $item_id){
            if ($time < $item_id->promotion_start_time && !empty($item_id->promotion_start_time)) {
                $arr_id .= $item_id->id . ',';
            } elseif ($time > $item_id->promotion_end_time && !empty($item_id->promotion_end_time)) {
        ...
        foreach ($data as $item) {
            ...
            if ($str_exist && strpos(',' . $str_exist . ',', ',' . $item->id . ',') !== false) {
                $html .= '<div class="red products_related_item  products_related_item_' . $item->id . ' '.$class.'" onclick="javascript: set_products_related(' . $item->id . ')" style="display:none" >';
                $html .= '<p style="margin-bottom: 0">'.$item->name . '</p>';
                if($item->promotion_id != 0){
                    $html .='<p style="font-size: 12px;margin-bottom: 0;display: inline-block">Đang thuộc chương trình khuyến mại: '.$item->promotion_name.' <span>('.$text.')</span></p>';
                }
//                $html .= '<img src="' . str_replace('/original/', '/resized/', URL_ROOT . @$item->image) . '" width="80">';
                $html .= '</div>';
            } else {
                $html .= '<div class="products_related_item  products_related_item_' . $item->id .  ' '.$class.'" onclick="javascript: set_products_related(' . $item->id . ')">';
                $html .= '<p style="margin-bottom: 0;display: inline-block">'.$item->name . '</p>';
                if($item->promotion_id){
                    $html .='<p style="font-size: 12px;margin-bottom: 0">Đang thuộc chương trình khuyến mại: '.$item->promotion_name.' <span>('.$text.')</span></p>';
                }
//                $html .= '<img src="' . str_replace('/original/', '/resized/', URL_ROOT . @$item->image) . '" width="80">';
                $html .= '</div>';
            }
