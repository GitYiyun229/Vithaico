...
class StoreControllersStock extends Controllers
{
    ...
    function news_genarate_related($data, $str_related = 0, $id)
    {
        ...
        foreach ($data as $item) {
            if ($str_exist && strpos(',' . $str_exist . ',', ',' . $item->id . ',') !== false) {
                ...
                $html .= $item->title . '<br/>';
                $html .= '<img style="height: 70px;width:100px;object-fit:contain;" onerror="' . $error_img . '" src="' . str_replace('/original/', '/small/', URL_ROOT . @$item->image) . '">';
                $html .= '</div>';
            } else {
...
class StoreControllersLocation extends Controllers
{
    function __construct()
    {
        $this->view = 'location';
