								...
								<tbody>
									...
									<?php if($count){?>
										<?php foreach ($list as $row) { ?>
											<?php $link_detail = FSRoute::_('index.php?module=products&view=cat&ccode='.$row->alias); ?>
							...
							<thead>
								<th class="title" >
									...
									<td width="30%">
										...
										<input type="hidden" name="product_id_current" id="product_id_current" value="0" />
...
//		$( "#text1" ).datepicker({clickInput:true,dateFormat: 'dd-mm-yy'});
	...
	function call_comments(){
		$('.product_record').click(function(){
			id = $(this).attr('id');
			product_id = id.replace('record_','');
			ajax_call_comments(product_id);
//			$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
//				$('#comments_area').html(html);
//				$('#unread_'+product_id).html(0);
//				redisplay_comments_static_on_tabhead(product_id);
//			});
			$('.product_record').removeClass('activated');
			$(this).addClass('activated');
		});
	}
	function ajax_call_comments(product_id){
		type = $('#type_comment').val();
		$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
			$('#comments_area').html(html);
			$('#unread_'+product_id).html(0);
			redisplay_comments_static_on_tabhead(product_id);
		});
	...
	function call_comments_by_type(){
		$('#comment_total_wrap').click(function(){
			product_id = $('#product_id_current').val();
			if(product_id != 0){
				$('#type_comment').val('0');
				ajax_call_comments(product_id);
	...
	function ajax_unpublished(id,product_id){
		$.get("index2.php?module=products&view=comments_categories&task=ajax_unpublished&raw=1",{id:id,product_id:product_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
//				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
				...
				no_published = $('#published_'+product_id).html();
				no_unpublished = $('#unpublished_'+product_id).html();
				no_published -- ;
				no_unpublished ++;
				$('#published_'+product_id).html(no_published);
				$('#unpublished_'+product_id).html(no_unpublished);
				redisplay_comments_static_on_tabhead(product_id);
			}
		});
	}
	...
	*/
	function redisplay_comments_static_on_tabhead(product_id){
		no_published = $('#published_'+product_id).html();
		no_unpublished = $('#unpublished_'+product_id).html();
		...
		$('#product_id_current').val(product_id);
		var title = $('#record_'+product_id).find('.title').html();
		console.log(title);
		$('#product_title').html(title);
		
	}
	function ajax_published(id,product_id){
		$.get("index2.php?module=products&view=comments_categories&task=ajax_published&raw=1",{id:id,product_id:product_id}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
//				$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
	...
	function ajax_del(id,product_id,published){
		if(confirm('Bạn chắc chắn muốn xóa?')){
			$.get("index2.php?module=products&view=comments_categories&task=ajax_del&raw=1",{id:id,product_id:product_id}, function(status){
				if(status == 1){
					type = $('#type_comment').val();
					$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
//					$.get("index2.php?module=products&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
	...
	function submit_reply(parent_id,product_id){
		...
		$.get("index.php?module=products&view=comments_categories&task=ajax_save_comment&raw=1",{product_id:product_id,parent_id:parent_id,name: name,text:reply_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
//				$.get("index2.php?module=product&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
					$('#comments_area').html(html);
				});
				no_published = $('#published_'+product_id).html();
				no_published ++ ;
	...
	function submit_edit(comment_id,product_id){
		edit_content = $('#text_edit_'+comment_id).val();
		if(edit_content == 'Nội dung' || edit_content == ''){
			alert('Bạn phải nhập nội dung');
			return false;
		}  
		$.get("index.php?module=products&view=comments_categories&task=ajax_edit_comment&raw=1",{product_id:product_id,comment_id:comment_id,name: name,text:edit_content}, function(status){
			if(status == 1){
				type = $('#type_comment').val();
				$.get("index2.php?module=products&view=comments_categories&task=ajax_get_comments_by_product&raw=1",{product_id:product_id,type:type}, function(html){
//				$.get("index2.php?module=product&view=comments&task=ajax_get_comments_by_product&raw=1",{product_id:product_id}, function(html){
