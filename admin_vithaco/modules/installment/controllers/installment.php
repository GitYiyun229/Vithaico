...
class InstallmentControllersInstallment extends Controllers
{
    function __construct()
    {
        $this->view = 'installment';
        parent::__construct();
        $array_status = array(
...
//var_dump($order);
        if ($order->warranty) {
            $warranty = $model->get_record('id= ' . $order->warranty, 'fs_warranty_price', 'warranty_name, price');
        }
        if ($order->aspect) {
            $aspect = $model->get_record('id = '.$order->aspect,'fs_aspect');
        }

        if ($order->store) {
            $store = $model->get_record('id= ' . $order->store, 'fs_address', 'name, address,phone');
        }
        if ($order->id_sub) {
            $product_sub = $model->get_record('id=' . $order->id_sub, 'fs_products_sub', 'image, name,price,price_old');
        }
        $product_main = $model->get_record('id=' . $order->product_id, 'fs_products', 'image,alias, name,price,price_old');

//        $data = $model->get_data_order_item();
//        var_dump($data);
//        $data = array();
//        $i =0;
...
//			$sender_province=$model->get_record_by_id($order->sender_province,'fs_cities')->name;
    ...
    function cancel_order()
    {
        ...
        $Itemid = 61;
        $id = FSInput::get('id');
        $link = 'index.php?module=installment&view=installment&task=edit&id=' . $id;
        if (!$rs) {
