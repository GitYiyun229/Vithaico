    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (!result.error) {
          ...
          if (id) {
            el.closest(".user-address")
              .find(".item-address")
              .attr("data-address", JSON.stringify(result.data));
            el.closest(".user-address")
              .find(".item-address div:first-child div:nth-child(1)")
              .text(result.data.name);
            el.closest(".user-address")
              .find(".item-address div:first-child div:nth-child(2)")
              .text(result.data.telephone);
            el.closest(".user-address")
              .find(".item-address div:first-child div:nth-child(3)")
              .text(
                `${result.data.address}, ${wardName}, ${districtName}, ${provinceName}`
              );
            if (result.data.default == 1) {
              el.closest(".user-address")
                .find(".item-address .text-nowrap")
                .html(
...
$(document).on("click", ".btn-cancel", function () {
  let data = JSON.parse(
    $(this).closest(".user-address").find(".item-address").attr("data-address")
  );

  $(this).closest(".user-address").find("form input[name=name]").val(data.name);
  $(this)
    .closest(".user-address")
    .find("form input[name=telephone]")
    .val(data.telephone);
  $(this)
    .closest(".user-address")
    .find("form input[name=address]")
    .val(data.address);
  $(this)
    .closest(".user-address")
    .find("form select[name=province]")
    .val(data.province_id)
    .trigger("change");
  $(this)
    .closest(".user-address")
    .find("form select[name=district]")
    .val(data.district_id)
    .trigger("change");
  $(this)
    .closest(".user-address")
    .find("form select[name=ward]")
    .val(data.ward_id)
    .trigger("change");
  $(this)
    .closest(".user-address")
    .find("form input[name=default]")
    .prop("checked", data.default == 1 ? true : false);
});
...
function validateAddress(el) {
  ...
  if (!name) {
    el.closest("form")
      ...
      .focus()
      .after('<div class="label_error text-red">Vui lòng nhập họ tên</div>');
    return false;
  }
  ...
  if (!telephone) {
    el.closest("form")
      ...
      .focus()
      .after(
        '<div class="label_error text-red">Vui lòng nhập số điện thoại</div>'
      );
    return false;
  }
  ...
  if (!regexTelephone.test(telephone)) {
    el.closest("form")
      ...
      .focus()
      .after(
        '<div class="label_error text-red">Số điện thoại không đúng định dạng</div>'
      );
    return false;
  }
  ...
  if (!address) {
    el.closest("form")
      ...
      .focus()
      .after('<div class="label_error text-red">Vui lòng nhập địa chỉ</div>');
    return false;
  }
  ...
  if (!province || province == 0) {
    el.closest("form")
      ...
      .select2("open")
      .after(
        '<div class="label_error text-red">Vui lòng chọn Tỉnh/ Thành phố</div>'
      );
    return false;
  }
  ...
  if (!district || district == 0) {
    el.closest("form")
      ...
      .select2("open")
      .after(
        '<div class="label_error text-red">Vui lòng chọn Quận/ Huyện</div>'
      );
    return false;
  }
  ...
  if (!ward || ward == 0) {
    el.closest("form")
      ...
      .select2("open")
      .after(
        '<div class="label_error text-red">Vui lòng chọn Phường/ Xã</div>'
      );
    return false;
  }
...
function resetAddress() {
  ...
  $(".modal-body-add form select[name=district]")
    .empty()
    .append(new Option("Quận/Huyện", 0, false, false))
    .trigger("change");
  $(".modal-body-add form select[name=ward]")
    .empty()
    .append(new Option("Phường/Xã", 0, false, false))
    .trigger("change");
}
...
function generateNewAddress(
  ...
  return `
        <div class="user-address position-relative mb-3 ">
            <div class="item-address p-3 d-flex align-items-end justify-content-between gap-3 position-relative" data-address='${JSON.stringify(
              result.data
            )}'>
                <div>
                    <div class="fw-medium">${result.data.name}</div>
                    <div class="fw-medium">${result.data.telephone}</div>
                    <div class="fw-medium">${provinceName}, ${districtName}, ${wardName}, ${
    result.data.address
  }</div>
                </div>
                <div class="text-nowrap">
                    ${
                      result.data.default == 1
                        ? `<div class="position-absolute text-grey is-default">Mặc định</div>`
                        : ``
                    }
                </div>
            </div>
            <a class="btn-show-form" data-bs-toggle="collapse" href="#addressForm${
              result.data.id
            }" role="button" aria-expanded="false" aria-controls="addressForm${
    result.data.id
  }">
                Sửa
            </a>
            <div class="collapse" id="addressForm${result.data.id}">
                <div class="card card-body">
                    <form action="" method="POST" class="modal-form">
                        ...
                        <div class="col">
                            <input type="tel" class="form-control" name="telephone" placeholder="Số điện thoại" value="${
                              result.data.telephone
                            }">
                        </div>
                        <div class="col">
                            <input type="text" class="form-control" name="address" placeholder="Địa chỉ (Ví dụ: Số 23, ngõ 66, hồ tùng mậu)" value="${
                              result.data.address
                            }">
                        </div>
                        <div class="col">
                        ...
                        <div class="col col-12 form-check">
                            <input class="form-check-input" type="checkbox" value="1" name="default" ${
                              result.data.default == 1 ? `checked` : ``
                            } id="default${result.data.id}">
                        ...
                        <div class="col col-12 d-flex align-items-center justify-content-between">
                            <a href="" class="fw-medium remove-address" data-id="${
                              result.data.id
                            }">Xóa địa chỉ</a>
                            <div class="d-flex align-items-center gap-3">
                                <a class="btn-form btn-cancel" data-bs-toggle="collapse" href="#addressForm${
                                  result.data.id
                                }" role="button" aria-expanded="false" aria-controls="addressForm${
    result.data.id
  }">
