...
function getOTP(telephone = '', email = '', _this)
{
    $.ajax({
        ...
        success: function (result) {
			console.log(result);
            if (result.error) {
                telephone ? invalid('telephone', result.message) : invalid('email', result.message);
            } else {
                telephone ? $('.telephone-change-text').text(telephone) : $('.email-change-text').text(email);
...
function saveChangeInfoDashboard(telephone, email, _this){
    let otp = [];
    let error = [];
    _this.closest('.modal-body').find('.form-otp').each(function(index){
    ...
    if (error.length) {
        _this.closest('.modal-body').find('.otp-message').text('Vui lòng nhập mã OTP');
        _this.closest('.modal-body').find('.form-otp:nth-child(' + error[0] + ')').focus();
        return false;
    }
...
function checkRulePassword(password, new_password, re_new_password){
    let updateRuleStatus = (selector, isValid) => {
        $(selector).addClass(isValid ? 'success' : 'error').removeClass(isValid ? 'error' : 'success');
    };
    ...
    return ($('.password-rule.error').length === 0 && password && re_new_password === new_password);
}
