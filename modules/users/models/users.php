...
class UsersModelsUsers extends FSModels
{
    ...
    function save()
    {
        ...
        $password_de = FSInput::get("r_password");
        $row['password'] = md5($password_de);
        $row['full_name'] = FSInput::get("name");
...
//			$body .= '<div><img src="http://pandabooks.vn/images/logos/logo_panda.jpg" alt="Pandabooks.vn logo"></div>';
    ...
    function mail_to_buyer($otp,$email)
    {
        if (!$otp)
            return;
        global $db;
...
//            $mail_order_body = $global->getConfig('mail_order_body');
        $mail_order_subject = "Mã xác nhận tài khoản 24hstore";
        $mailer->isHTML(true);
        $mailer->setSender(array($admin_email, $admin_name));
        $mailer->AddBCC($admin_email, $admin_name);
        $mailer->AddAddress($email, "Quý khách");
        $mailer->setSubject($mail_order_subject);
        $body = '<p>Mã xác nhận tài khoản của bạn là: ' . $otp . '</p>';
